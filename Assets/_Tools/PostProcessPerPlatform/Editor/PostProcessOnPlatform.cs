...
public class PostProcessOnPlatform : ScriptableObject, IProcessSceneWithReport, IPreprocessBuildWithReport
{
    ...
    [Header("Default")]
    [SerializeField] VolumeProfile[] defaultVolumes;
    private static VolumeProfile[] staticDefaultVolumes;

    [SerializeField] VolumesPlatform[] volumesOnPlatforms;
    private static VolumesPlatform[] staticVolumesPlatforms;

    public static int plataform = -1;
    public int callbackOrder => 0;

    private static PostProcessOnPlatform postProcessOnPlatform;
    ...
    {
        if (BuildPipeline.isBuildingPlayer == false || plataform == -1)
        {
            return;
        }

        Volume[] sceneVolumes = FindObjectsOfType<Volume>(true);
        GetPlatformVolume(sceneVolumes);
        Debug.Log($"Updated {sceneVolumes.Length} volumes in scene {scene.path}");
    }
