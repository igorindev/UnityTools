...
namespace Gma.DataStructures.StringSearch
{
    public class UkkonenTrie<K, T> : IGenericSuffixTrie<K, T> where K : IEquatable<K>, IComparable<K>
    {
        ...
        private static (bool, Node<K, WordPosition<T>>, int) TestAndSplit(Node<K, WordPosition<T>> inputs, StringSlice<K> stringPart, K t, ReadOnlyMemory<K> remainder, T value, int size, int offset)
        {
            ...
            (Node<K, WordPosition<T>> s, StringSlice<K> str, int offsetResult) = Canonize(inputs, stringPart, offset);
            offset = offsetResult;
            ...
            Edge<K, WordPosition<T>> e = s.GetEdge(t);
            if (null == e)
            {
                // if there is no t-transtion from s
                return (false, s, offset);
            }
            ReadOnlySpan<K> eLabelSpan = e.Label.Span;
            ReadOnlySpan<K> remainderSpan = remainder.Span;
